...
function extendPathFunctions() {
  paper.Path.inject({

    /////////////////////////////////////
    //     Elastic path functions
    /////////////////////////////////////

    adjustSegments: function(numSegments) {
      if (this.segments.length == numSegments) {
        return;
      }

      var toDivide = numSegments - this.segments.length,
          length = this.length,
          location;
      for (var i = 0; i < toDivide; i++) {
        location = this.getLocationAt(length * i / toDivide + 0.001);   // [FIXME] `0.001` <- dirty hack
        this.curves[location.index].divideAt(location);
      }

      console.assert(
        (this.segments.length == numSegments),
        {msg: "adjustSegments not matched",
         numSegments: numSegments,
         segments: this.segments.length});
    },

    hasMorph: function(name) {
      return this.morphDict && (name in this.morphDict);
    },

    registerMorph: function(name, path) {
      if (this.morphDict === undefined) {
        this.morphDict = {};
      }
      this.morphDict[name] = path;
    },

    adaptTransition: function(from, to, ratio) {
      var before = (from in this.morphDict) ? this.morphDict[from]: this.morphDict['base'],
          after  = (to   in this.morphDict) ? this.morphDict[to]  : this.morphDict['base'],
          newSegment;

      for (var i = 0; i < after.segments.length; i++) {
        newSegment = segmentProportion(before.segments[i], after.segments[i], ratio);
        this.segments[i].point = newSegment.point;
        this.segments[i].handleIn = newSegment.handleIn;
        this.segments[i].handleOut = newSegment.handleOut;
      }
    },
