...
function EyeController(tool, chino) {
  var _this = this;
  this.tool = tool;
  this.chino = chino;
  this.tool.onMouseMove = function(e) {
    _this.moveEyes(e.point);
  };
}
EyeController.prototype.moveEyes = function(point) {
  /*
    [FIXME] more sophisticated method
      ...
      x=190           x=375
   */
  this.chino.eyes[0].setIrisPos(point.divide(400).add(-0.5, -2.8));
  this.chino.eyes[1].setIrisPos(point.divide(400).add(-0.5, -2.8));
  this.chino.eyes[0].setPupilPos(point.divide(100).add(-1.5, -1.5));
  this.chino.eyes[1].setPupilPos(point.divide(100).add(-1.5, -1.5));
}
...
function MangaEye(direction) {
  ...
  this.children = {};
  this.irisPos = new Point(0, -1);
  this.pupilPos = new Point(0, 0);
}

MangaEye.prototype.setIrisPos  = function(p) { this.irisPos  = p; };
MangaEye.prototype.setPupilPos = function(p) { this.pupilPos = p; };
...
MangaEye.prototype.renderIris = function() {
  ...
  baseIris.position = baseIris.position.add(this.irisPos.x, this.irisPos.y);
...
MangaEye.prototype.renderPupil = function() {
  ...
  var pupilb = new Path.Ellipse({
    point: this.children.baseIris.position.add(- pw * R * 0.5 * iw + xd, -ph * R * 0.5 * ih + 2).add(this.pupilPos),
    size: this.children.baseIris.bounds.scale(pw * R, ph * R)
  });
  pupilb.fillColor = pupilColor(pupilb, 120);

  var pupil = new Path.Ellipse({
    point: this.children.baseIris.position.add(- pw * R * 0.5 * iw + xd, -ph * R * 0.5 * ih + 4).add(this.pupilPos),
    size: this.children.baseIris.bounds.scale(pw * R, ph * R)
  });
...
Character.prototype.render = function() {
  ...
  if (this.eyes.length == 0) {
    this.eyes.push(new MangaEye("l"));
    this.eyes.push(new MangaEye("r"));
  }
  this.renderEyes();
