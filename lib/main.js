...
function CharacterConfig() {
  ...
  this.durationTransition = 5.0;
  this.keyframeTransition = [
    {x:0,    y:0,    l: 0,     r:0.05},
    {x:0.1,  y:1.0,  l:-0.05,  r:0.0},
    {x:1.0,  y:1.0,  l:-0.5,   r:0.0},
  ];
  this.durationBlink = 3.0;
  this.keyframeBlink = [
    {x:0,    y:0,   l: 0,    r:0},
    {x:0.63, y:0,   l:-0.02, r:0.02},
    {x:0.65, y:1.0, l:-0.02, r:0.02},
    {x:0.67, y:0,   l:-0.02, r:0.02},
...
window.onload = function() {
  ...
  gui.add(config, 'faceAngle', -3.2, 3.2);
  ...
  $.getJSON("references/facial-expressions.json", function(data) {
    ...
    chino.addExpression('surprise', data['chino-bezier-surprise.svg']);
  });

  var tool = new Tool();
  var eyeControl = new EyeController(tool, chino);

  var ExpressionState = function() {
    this.index = 0;
    this.states = Object.freeze(['peace', 'angry', 'peace', 'surprise', 'peace', 'sad']);
  };
